...
def priest(history, memory):
    ...
    BEJOSS = 7
    memory[COOLDOWN] +=1
    if memory[BEJOSS] and memory[COOLDOWN]%6 ==0:
        if history[1, -1]:
            return "defect", memory
        else:
            memory[COOLDOWN]-=6
    if memory[BEJOSS] and memory[COOLDOWN] ==8:
        if history[1, -1]:
            memory[COOLDOWN]=2
    if memory[BEJOSS] and memory[COOLDOWN] == 14:
        if history[1, -1]:
            memory[COOLDOWN]=2
        else:
            memory[BEJOSS] = False
...
def notDefectedAgainst(history):
    for x in range(0,history.shape[1]):
        if history[1,x] == 0:
            return 0
    return 1
...
def strategy(history, memory):
    ...
    if round == 7:
        ...
        for i in range (1, 8):
            sin+=1-history[1,-i]
        if sin == 7:
            memory[TACTIC] = "grudged"

    if notDefectedAgainst(history):
        return "cooperate", memory
    if memory[TACTIC] == "grudged":
        return grudged(history, memory)
    if 3<=round<=delay:
        return forgivingCopycat(history), memory
    if round == delay+1:
        sin = 0
